...
def tabla_delivery(request):
    ...
    lista_tickets = Ticket.objects.filter(caja__in=filtros, tipo="DELIVERY").order_by(
        "-add_time"
    )
...
def tabla_cobrado(request):
    ...
    cobrado_total = sum(ticket.get_total() for ticket in lista_tickets)
    paginator = Paginator(lista_tickets, 50)
    page_number = request.GET.get("page", 1)
    tickets = paginator.get_page(page_number)
    context = {
...
def ver_tabla_por_mesa(request, mesa=None):
    ...
    lista_mesas = Ticket.objects.filter(caja__in=filtros_mesa, tipo="MESA").order_by(
        "mesa"
    )
    numMesaUnico = (
        Ticket.objects.filter(caja__in=filtros_mesa, tipo="MESA")
        .values("mesa")
        .distinct()
        .order_by("mesa")
    )
    if mesa != None:
        mesaNum = int(mesa)
        lista_mesa_num = Ticket.objects.filter(
            mesa=mesaNum, tipo="MESA", caja="PENDIENTE"
        ).order_by("-add_time")
